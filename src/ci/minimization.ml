  ...
  | Some _ -> (
      ...
      | Ok
          ...
          ( match jobs_to_minimize with
            ...
            | _ ->
                Lwt_io.printlf
                  "Initiating CI minimization for PR #%d on jobs: %s" pr_number
                  ( jobs_to_minimize
          ...
          | Ok (jobs_minimized, jobs_that_could_not_be_minimized) -> (
              ...
              let bad_jobs_description ~f =
                ...
                | [(reason, {target})] ->
                    Some
                      (Printf.sprintf
                         "The job %s was not minimized because %s.\n" target
                         reason )
                | bad_jobs ->
                    Some
              ...
              let unfinished_pipelines_description =
                ( if base_pipeline_finished then []
                  else [f "base commit (%s)" base] )
                @
                if head_pipeline_finished then []
                else [f "head commit (%s)" head]
              in
              let try_again_msg =
                ...
                | ls ->
                    f
                      "\n\
                       However, you may want to try again once the %s for the \
                       %s %s."
              ...
              let may_wish_to_wait_msg =
                ...
                | ls ->
                    f
                      "\n\n\
                       :warning: :hourglass: You may want to wait until the %s \
                       for the %s %s."
                      (pluralize "pipeline" ls)
                      (ls |> String.concat ~sep:" and ")
                      (pluralize "finishes" ~plural:"finish" ls)
              ...
              let note_some_head_unfinished_msg =
                if head_pipeline_finished then ""
                  ...
                  f
                    "\n\
                     Some jobs may have been missed because the pipeline for \
                     the head commit (%s) has not yet finished."
                    head
              in
              let note_some_base_unfinished_msg =
                if base_pipeline_finished then ""
                  ...
                  f
                    "\n\
                     However, minimization may fail because the pipeline for \
                     the base commit (%s) has not yet finished."
                    base
              in
              ( match
                  (request, jobs_minimized, failed_minimization_description)
                with
                | RequestAll, [], None ->
                    Lwt.return_some
                      ( match
                          bad_and_unminimizable_jobs_description ~f:(fun _ ->
                              true )
                        with
                      | None ->
                          f "No valid CI jobs detected for %s.%s" head
                            try_again_msg
                      | Some msg ->
                          f
                ...
                | RequestAll, _, _ ->
                    ( match
                      ...
                      | Some msg ->
                          Lwt_io.printlf
                            "When attempting to run CI Minimization by request \
                             all on %s/%s@%s for PR #%d:\n\
                             %s"
                    ...
                    ( match jobs_minimized with
                      | [] ->
                          f
                            "I did not succeed at triggering minimization on \
                             any jobs at commit %s.%s"
                            head try_again_msg
                      | _ :: _ ->
                          ...
                          f
                            "I am now %s at commit %s on %s. I'll come back to \
                 ...
                 library:ci-cross_crypto (pull request) *)
                    ...
                    |> List.partition3_map ~f:(fun request ->
                        ...
                        | false, Some (target, err), _, _ ->
                            `Snd
                              (f "%s: failed to trigger minimization (%s)"
                                 target err )
                    ...
                    let unfound_requests_report =
                      ...
                      | [request] ->
                          Some
                            (f
                               "requested target '%s' could not be found among \
                                the jobs %s.%s"
                               request
                      ...
                      | _ :: _ :: _ ->
                          Some
                            (f
                               "requested targets %s could not be found among \
                                the jobs %s.%s"
                    ...
                    in
                    ( match
                        (successful_requests, unsuccessful_requests_report)
                      with
                      | [], None ->
                          "No CI minimization requests made?"
                      | [], Some msg ->
                          "I was unable to minimize any of the CI targets that \
                           you requested." ^ try_again_msg ^ "\n" ^ msg
                      | _ :: _, _ ->
                          (* TODO: change https://github.com/rocq-community/run-coq-bug-minimizer/actions to a link to the particular action run when we can get that information *)
                          f
                            "I am now %s at commit %s on requested %s %s. I'll \
                             come back to you with the results once it's \
                             done.%s\n\n\
                             %s"
                            (minimizer_status_link ~minimizer_url
                               ~verb:
                            ...
                            (Option.value ~default:""
                               unsuccessful_requests_report ) )
                    |> Lwt.return_some
                | RequestSuggested, [], None ->
                    ( match possible_jobs_to_minimize with
                      | [] ->
                          f
                            "No CI jobs are available to be minimized for \
                             commit %s.%s"
                            head try_again_msg
                      | _ :: _ ->
                          f
                            "You requested minimization of suggested failing \
                             CI jobs, but no jobs were suggested at commit %s. \
                             You can trigger minimization of %s with `ci \
                             minimize all` or by requesting some targets by \
                             name.%s"
                            head
                            ( possible_jobs_to_minimize
                ...
                | RequestSuggested, [], Some failed_minimization_description ->
                    f
                      "I attempted to minimize suggested failing CI jobs at \
                       commit %s, but was unable to succeed on any jobs.%s\n\
                       %s"
                ...
                | Auto, jobs_minimized, failed_minimization_description -> (
                    ...
                    let suggest_only_all_jobs =
                      let pre_message =
                        f
                          "- If you tag me saying `@%s ci minimize all`, I \
                           will additionally minimize the following %s (which \
                           I do not suggest minimizing):"
                          bot_info.github_name
                    ...
                    | [], None, None, None, _ ->
                        Lwt_io.printlf
                          "No candidates found for minimization on %s/%s@%s \
                           for PR #%d."
                    ...
                    | [], None, Some suggestion_msg, _, Error reason ->
                        Lwt_io.printlf
                          "Candidates found for minimization on %s/%s@%s for \
                           PR #%d, but I am not commenting because \
                           minimization is not suggested because %s:\n\
                           %s\n\
                           %s"
                    ...
                    | [], Some failed_minimization_description, _, _, _ ->
                        Lwt_io.printlf
                          "Candidates found for auto minimization on %s/%s@%s \
                           for PR #%d, but all attempts to trigger \
                           minimization failed:\n\
                           %s"
                          owner repo head pr_number
                          failed_minimization_description
                    ...
                    | [], None, Some suggestion_msg, _, Ok () ->
                        f
                          ":red_circle: CI %s at commit %s without any failure \
                           in the test-suite\n\n\
                           :heavy_check_mark: Corresponding %s for the base \
                           commit %s succeeded\n\n\
                           :grey_question: Ask me to try to extract %s that \
                           can be added to the test-suite\n\n\
                           <details><summary>%s</summary>\n\n\
                           - You can also pass me a specific list of targets \
                           to minimize as arguments.\n\
                           %s\n\
                           </details>%s"
                    ...
                    | _ :: _, _, _, _, _ ->
                        f
                          ":red_circle: CI %s at commit %s without any failure \
                           ...
                           :heavy_check_mark: Corresponding %s for the base \
                           commit %s succeeded\n\n\
                           <details><summary>:runner: I have automatically \
                           started minimization for %s to augment the \
                           test-suite</summary>\n\n\
                           - You can also pass me a specific list of targets \
                           to minimize as arguments.\n\
                           %s\n\
              ...
              | Some message ->
                  Bot_components.GitHub_mutations.post_comment
                    ~id:comment_thread_id ~message ~bot_info
                  >>= Utils.report_on_posting_comment
              | None ->
                  Lwt_io.printlf
                    "NOT commenting with CI minimization information at \
                     %s/%s@%s (PR #%d)."
                    owner repo head pr_number )
          | Error err ->
