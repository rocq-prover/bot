...
let gitlab_job_url ~gitlab_domain ~gitlab_owner ~gitlab_repo ~job_id =
...
let bench_summary_job_link_prefix ~gitlab_domain ~gitlab_owner ~gitlab_repo
    ~job_name =
  match (gitlab_domain, gitlab_owner, gitlab_repo) with
  | Some domain, Some owner, Some repo ->
...
let bench_summary_build_id_regex ~gitlab_domain ~gitlab_owner ~gitlab_repo
    ~job_name =
  Option.map
    (bench_summary_job_link_prefix ~gitlab_domain ~gitlab_owner ~gitlab_repo
       ~job_name ) ~f:(fun prefix -> f {|.*%s\([0-9]*\)|} (Str.quote prefix) )

let update_bench_status ~bot_info ~job_info (gh_owner, gh_repo) ~external_id
    ~number ~gitlab_domain ~gitlab_owner ~gitlab_repo =
  let open Lwt.Syntax in
  match number with
  | None ->
  ...
  | Some number -> (
    match
      gitlab_job_url ~gitlab_domain ~gitlab_owner ~gitlab_repo
        ~job_id:job_info.build_id
    with
    | None ->
        Lwt_io.printlf
          "No GitLab coordinates configured for %s/%s bench status; skipping"
          gh_owner gh_repo
    | Some gitlab_url -> (
...
let run_bench ~bot_info ?(org = "rocq-prover") ?(team = "contributors")
    ~gitlab_domain ~gitlab_owner ~gitlab_repo ~bench_job ?key_value_pairs
    comment_info =
  ...
  let missing_coords_msg =
    f
      "Cannot run bench for %s/%s#%d: GitLab coordinates are not configured \
       (gitlab_domain, gitlab_owner, gitlab_repo)."
      owner repo pr_number
  in
  match
    bench_summary_build_id_regex ~gitlab_domain ~gitlab_owner ~gitlab_repo
      ~job_name:bench_job
  with
  | None ->
      GitHub_mutations.post_comment ~bot_info ~message:missing_coords_msg
        ~id:pr.id
      >>= Utils.report_on_posting_comment
  | Some build_id_regexp -> (
     ...
     order to get our hands on this information. TODO: do this more directly.*)
      let* gitlab_check_summary =
        ...
        | Error err ->
            Lwt.return_error
              (f
                 "Error while fetching PR refs for %s/%s#%d for running bench \
                  job: %s"
      ...
      let* process_summary =
        ...
        | Ok summary -> (
          try
            let build_id =
              ( if String_utils.string_match ~regexp:build_id_regexp summary then
                  Str.matched_group 1 summary
                else
                  raise @@ Stdlib.Failure "Could not find GitLab bench job ID"
              )
          ...
          with Stdlib.Failure s ->
            Lwt.return_error
              (f
                 "Error while regexing summary for %s/%s#%d for running bench \
                  job: %s"
      ...
      in
      match (allowed_to_bench, process_summary, gitlab_domain) with
      | Ok true, Ok (build_id, project_id), Some domain ->
          (* Permission to bench has been granted *)
          GitLab_mutations.play_job ~bot_info ~gitlab_domain:domain ~project_id
            ~build_id ?key_value_pairs ()
      | Ok true, Ok _, None ->
          GitHub_mutations.post_comment ~bot_info ~message:missing_coords_msg
            ~id:pr.id
          >>= Utils.report_on_posting_comment
