...
let fetch_bench_results ~repo_config ~job_info () =
  let open BenchResults in
  let open Lwt.Syntax in
  match
    Repo_config.gitlab_pages_artifact_url repo_config ~job_id:job_info.build_id
  ...
  | Some _ ->
      let artifact_url file =
        ...
        Option.value_exn
          (Repo_config.gitlab_pages_artifact_url repo_config
             ~job_id:job_info.build_id
             ~artifact:(f "_bench/timings/%s" file) )
      in
      ...
      in
      let* slow_table =
        let* slow_table_or_err =
          artifact_url "slow_table.html" |> fetch_artifact
        in
        ...
        | Error _ -> (
            let* slow_table_or_err =
              artifact_url "slow_table" |> fetch_artifact
            in
            match slow_table_or_err with
            | Ok s ->
      ...
      in
      let* fast_table =
        let* fast_table_or_err =
          artifact_url "fast_table.html" |> fetch_artifact
        in
        ...
        | Error _ -> (
            let* fast_table_or_err =
              artifact_url "fast_table" |> fetch_artifact
            in
            match fast_table_or_err with
            | Ok s ->
                Lwt.return (String_utils.code_wrap s)
...
let update_bench_status ~bot_info ~repo_config ~job_info (gh_owner, gh_repo)
    ~external_id ~number =
  let open Lwt.Syntax in
  match number with
  | None ->
  ...
  | Some number -> (
    match Repo_config.gitlab_job_url repo_config ~job_id:job_info.build_id with
    | None ->
        Lwt_io.printlf
          "Skipping bench status: gitlab_domain/owner/repo missing in \
           repo_config."
    | Some gitlab_url -> (
        ...
        | Ok repo_id -> (
            ...
            match state with
            | "success" ->
                let* results = fetch_bench_results ~repo_config ~job_info () in
...
let run_bench ~bot_info ~repo_config ~team ?key_value_pairs comment_info =
  let open Lwt.Syntax in
  ...
  match
    ( repo_config.gitlab_domain
    , repo_config.gitlab_owner
    , repo_config.gitlab_repo
    , repo_config.jobs.bench_job )
  with
  ...
  | Some gitlab_domain, Some gitlab_owner, Some gitlab_repo, Some bench_job ->
      ...
      | Ok true, Ok (build_id, project_id) ->
          GitLab_mutations.play_job ~bot_info ~gitlab_domain ~project_id
